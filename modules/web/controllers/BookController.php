...
class BookController extends BaseController{
	...
	public function actionSet(){
		...
		if( !$cat_id ){
			return $this->renderJSON([],"请输入线路分类~~",-1);
		}

		if( mb_strlen( $name,"utf-8" ) < 1 ){
			return $this->renderJSON([],"请输入符合规范的线路名称~~",-1);
		}

		if( $price <= 0  ){
			return $this->renderJSON([],"请输入符合规范的线路预售定金~~",-1);
		}
		...
		if( mb_strlen( $summary,"utf-8" ) < 10 ){
			return $this->renderJSON([],"请输入线路描述，并不能少于10个字符~~",-1);
		}

		if( $stock < 1 ){
			return $this->renderJSON([],"请输入符合规范的剩余量~~",-1);
		}

		if( mb_strlen( $tags,"utf-8" ) < 1 ){
			return $this->renderJSON([],"请输入标签，便于搜索~~",-1);
		}
	...
	public function actionOps(){
		...
		$info = Book::find()->where([ 'id' => $id ])->one();
		if( !$info ){
			return $this->renderJSON([],"指定线路不存在~~",-1);
		}
