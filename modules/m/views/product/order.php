...
<div class="order_box">
		...
		<?php if( $address_list ):?>
            ...
            <li style="padding: 5px 5px;">
                <label>
                    ...
                    <?=$_address_info['address'];?>（<?=$_address_info['nickname'];?>乘车）<?=$_address_info['mobile'];?>
                </label>

            </li>
            <?php endforeach;?>
		<?php else:?>
            <li style="padding: 5px 5px;"> <a href="<?=UrlService::buildMUrl('/user/address_set');?>">快去添加接送地址啦~~</a></li>
		<?php endif;?>
    </ul>
	...
	<ul class="order_list">
		...
		<li data="<?=$_item["id"];?>" data-quantity="<?=$_item['quantity'];?>" data-chuxingtime="<?=$_item['chuxingtime'];?>">
