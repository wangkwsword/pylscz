...
class PayOrderService extends  BaseService {
	...
	public static function createPayOrder( $member_id,$items = [],$params = []){
		...
		try{
			...
			$model_pay_order->chuxingtime = isset($params['chuxingtime'])?$params['chuxingtime']:'123';
//            $model_pay_order->chuxingtime = '123';
			...
			foreach($items as $_item){
				...
				if( $tmp_left_stock < $_item['quantity'] ){
					throw new Exception("剩余座位不足,目前剩余：{$tmp_left_stock},你预订了:{$_item['quantity']}~~");
				}
